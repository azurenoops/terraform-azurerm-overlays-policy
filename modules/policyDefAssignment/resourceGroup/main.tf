...
resource "azurerm_role_assignment" "rem_role" {
  ...
  principal_id                     = azurerm_resource_group_policy_assignment.def[0].identity[0].principal_id
...
resource "azurerm_resource_group_policy_remediation" "rem" {
  ...
  policy_assignment_id    = azurerm_resource_group_policy_assignment.def[0].id

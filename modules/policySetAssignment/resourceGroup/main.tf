...
resource azurerm_role_assignment rem_role {
  ...
  principal_id                     = azurerm_resource_group_policy_assignment.set[0].identity[0].principal_id
  skip_service_principal_aad_check = true
}

## remediation tasks ##
resource azurerm_resource_group_policy_remediation rem {
  for_each                       = { for dr in local.local.definition_reference.rg : basename(dr.reference_id) => dr }
  name                           = lower("${each.key}-${formatdate("DD-MM-YYYY-hh:mm:ss", timestamp())}")
  resource_group_id              = local.remediation_scope
  policy_assignment_id           = azurerm_resource_group_policy_assignment.set[0].id
